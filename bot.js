...
client.user.setGame(`DreamCraft l +help`,"http://twitch.tv/S-F")
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
:crown:${member}:crown:
https://www.askideas.com/media/13/Beautiful-Wooden-Welcome-Sign.jpg
...
client.on('message', message => {
  ...
  const embed = new Discord.RichEmbed()
  .setTitle('Click here')
  .setURL('https://discord.gg/qMhZcHz ')
  .setColor('RANDOM')
...
if(message.content.startsWith(prefix + 'color')) {
   ...
   client.on("message", msg => {
           var prefix = "+";
  if(msg.content.startsWith (prefix + "id-server")) {
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("463277981146808331");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("499935158443704330");
    if (!channel) {
